...
function metadataFor(candidate) {
  ...
  const groups = {
    ...
    "recruit:interview": { tags: ["採用", "面談", "職員", "対話", "相談"], scene: "採用面談", themes: ["recruit"], technology_level: "none" },
  ...
  const named = candidate.path.includes("budget") ? { tags: ["補助金", "予算", "計算", "書類"], scene: "予算検討", themes: ["subsidy"], technology_level: "none" } :
    candidate.path.includes("consultation") ? { tags: ["補助金", "相談", "申請", "書類"], scene: "補助金相談", themes: ["subsidy"], technology_level: "none" } :
    candidate.path.includes("website") ? { tags: ["補助金", "ホームページ", "相談", "書類"], scene: "補助金相談", themes: ["subsidy"], technology_level: "none" } : null;
...
function refreshImageMetadata({ root = path.join(__dirname, ".."), kind = "hero" } = {}) {
  ...
  const candidates = images.discoverImages({ root, kind });
  const catalog = candidates.map(candidate => byPath.get(candidate.path) || metadataFor(candidate));
  const document = { version: 1, generated_for: "hero", images: catalog };
