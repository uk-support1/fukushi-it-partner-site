...
async function generateArticle({apiKey, model, localDate, topic, request = requestGemini}) {
  ...
  const instructions = "あなたは福祉ITパートナーの編集・執筆担当です。選定済みテーマから、日本語の実務的なブログ記事を作成してください。" +
    ...
    "タイトルは入力されたtopic.titleを一字一句変えません。front matter、画像、URL、Markdownリンク、関連記事、日付、slugは出力しません。" +
    "存在しない制度、法律、補助金、自治体、サービス事例、URL、根拠のない数値や統計を作りません。制度、法律、補助金、報酬改定、金額、期限など最新性の確認が必要な事項は、確認済みの一次情報が入力にないため一般論に留め、断定しません。";
