...
概要・見出しを読み、YouTube4チャンネルの動画候補（後述）を収集して、Gemini APIの
generateContentでテーマ選定と独立した重複確認を行います。
...
`scripts/latest-info.js` は、厚生労働省、福祉医療機構（WAM NET）、デジタル庁、内閣府の
公式RSSを取得する機能を持っています。WAM NETは障害福祉サービス等報酬改定や介護保険部会など、
福祉事業者に直結する審議会・行政資料を配信しており、利用者に関心の薄い一般的な省庁ニュース
だけに偏らないようにするための情報源として追加したものです。

**ただし2026-09-24の仕様変更以降、Daily Blogの通常実行はこれらの公式情報源を使いません**
（後述の「YouTubeチャンネルの動画情報」だけを候補とします）。テーマを毎回、対象4チャンネルの
動画発信者個人の見解を紹介する形に統一するための方針です。公式RSS取得のコード・テストは
削除せず残しており、`scripts/latest-info.js`を直接呼び出せば従来通り動作します。将来また
公式情報源を組み合わせたくなった場合は、`scripts/daily-blog.js`の`sources`の組み立て方を
戻すだけで復元できます。Yahoo!ニュース等の一般ニュースサイトは、RSS・掲載情報の商用利用や
それを用いたプログラムでの二次利用を利用規約で明確に禁止しているため、そもそも情報源には
採用していません。
...
（`youtube.com/feeds/videos.xml?channel_id=...`）を候補として扱います。**Daily Blogの
通常実行は、テーマ候補としてこの4チャンネルの動画だけを使い、公式情報源（前項）は使い
ません**（2026-09-24〜）。これはGoogleが提供する公式機能で、Yahoo!ニュースのような
商用利用禁止規約はありません。取得できるのは
...
加えて、記事内の「当社見解」部分では、動画の発信者が述べている具体的な内容に一度触れた
うえで、「福祉ITパートナーとしても、動画内で〇〇さんが指摘しているように〇〇になっていくと
感じており、そのお手伝いをしていきたい」という形で、共感する点・今後の見通し・当社として
力になりたい姿勢を一言添えるよう指示します（会社としての声で統一し、発信者の発言をそのまま
繰り返さず、誇張した断定もしません）。動画以外の出典（通常の公式情報源）の記事にはこの指示は
適用されません。
...
動画1本の添付は入力トークンを約6.5万〜9万消費します（実測）。Gemini無料枠の
トークン上限は1分あたりの制限のため、記事生成のスタイル・重複見出しリトライ
（最大3回）のたびに動画を添付し直すと、同じ1分以内に上限（25万トークン）へ
到達し、Daily Blog全体が`RESOURCE_EXHAUSTED`で失敗する事例が本番で発生しました
（2026-09-19、2026-09-22）。そのため動画は**1回目の生成試行でのみ**添付し、
2回目以降のリトライでは添付しません（`scripts/article-generator.js`）。リトライは
すでに動画の内容を踏まえて書かれた`previousArticle`のテキストを見出し・文体の
観点で書き直すだけなので、動画への再アクセスは不要という判断です。
...
Daily Blog自体の毎日の公開は一切影響を受けません。動画候補が3件に満たない日（対象
チャンネルの更新が少ない、キャッシュが古いなど）は、既存の「候補3件未満なら通常テーマへ
自動フォールバック」がそのまま効き、動画に関係ない福祉IT一般のテーマで1件は必ず公開します。

YouTube側の取得だけを切り分けて確認したい場合は、Actionsの「Run workflow」から
`sources_filter`を`live`にしてDaily Blogを手動実行すると、その回だけYouTubeの
4チャンネルへ直接ライブ接続を試せます（`DAILY_BLOG_SOURCES_FILTER`環境変数）。通常の
スケジュール実行は常にキャッシュ経由（`cache`、既定値）です。
...
出典に`kind: "video"`の動画が含まれる記事は、記事生成の直後に
`scripts/lib/video-thumbnail.js`のfetchVideoThumbnail()がその動画のサムネイル
（`https://i.ytimg.com/vi/<videoId>/hqdefault.jpg`、YouTube公式のサムネイルCDNで、
このような埋め込み・カバー画像用途を想定した公式機能）を1回だけ取得し、
`assets/images/blog-library/hero-video/<videoId>.jpg`としてローカルへ保存します
（他の画像と同じく、外部URLを直接貼らずローカル保存して配信します）。このディレクトリは
既存の`assets/images/blog-library/hero/`とは別で、`discoverImages()`（記事一覧の
隣接カード同士でシリーズ・シーンが重ならないよう管理している既存の画像カタログ機構）
からは見えません。動画のサムネイルは元々その動画専用の一意な画像であり、使い回して
分類・多様性管理する対象ではないためです（`tests/image-library.test.js`で、
公開済み記事一覧のシリーズ・シーン隣接チェックからこのディレクトリを除外することを検証済み）。

取得できたサムネイルは`scripts/article-writer.js`のprecomputedHeroとしてカバー画像に
...
念のためtry/catchで包んでいます）。この場合は既存の通常のカバー画像選定（キュレーション
済みのストック画像ライブラリ）へそのまま自動的にフォールバックし、Daily Blog全体を
失敗させることはありません。動画以外の出典の記事（通常テーマ・公式情報源のみの記事）は
この処理自体が発生しません。

なお、GitHub Actionsのクラウドランナーから`i.ytimg.com`への到達性は本番未検証です
（YouTubeの動画一覧フィード取得だけがクラウドランナーからブロックされることを確認済みで、
画像CDNの`i.ytimg.com`は別ホストのため無関係の可能性が高いですが未確認）。仮にブロックされて
いたとしても、上記フォールバック設計により通常のストック画像へ切り替わるだけで、記事生成や
公開自体は失敗しません。
